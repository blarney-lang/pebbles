...
executeIxCHERI m_shiftUnit m_csrUnit m_memReqs s = do
  ...
  let cA = s.capA.capMem
  let cB = s.capB.capMem
  let topA = s.capA.capTop
  let baseA = s.capA.capBase
  let lenA = s.capA.capLength
  let addrA = getAddrCapMem cA
  let permsA = getHardPermsCapMem cA
  let permsB = getHardPermsCapMem cB
  ...
  when (s.opcode `is` [CSetFlags, CSetAddr, CIncOffset]) do
    when (isValidCapMem cA .&&. isSealedCapMem cA) do
      trap s cheri_exc_sealViolation

  when (s.opcode `is` [CSealEntry, CAndPerm]) do
    if inv (isValidCapMem cA) then
      trap s cheri_exc_tagViolation
    else if isSealedCapMem cA then
      trap s cheri_exc_sealViolation
  ...
  when (s.opcode `is` [CMove, CSealEntry, CSetFlags, CClearTag, CAndPerm]) do
    let newType  = if s.opcode `is` [CSealEntry]
                     then -2
                     else getTypeCapMem cA
    let newValid = if s.opcode `is` [CClearTag]
                     then 0
                     else isValidCapMem cA
    let newFlags = if s.opcode `is` [CSetFlags]
                     then lower s.opB
                     else getFlagsCapMem cA
    let newPerms = if s.opcode `is` [CAndPerm]
                     then getPermsCapMem cA .&. lower s.opB
                     else getPermsCapMem cA
    s.resultCapMem <==
  ...
  let newAddr0 = s.immOrOpB +
        (if s.opcode `is` [CSetAddr] then 0 else getAddrCapMem oldCap)
  ...
  let newCap = setValidCapMem (setAddrUnsafeCapMem oldCap newAddr1)
                              (isValidCapMem oldCap .&&. inv oob)

  when (s.opcode `is` [CSetAddr, CIncOffset, AUIPC]) do
    s.resultCapMem <== newCap
  ...
  when (s.opcode `is` [JAL, JALR]) do
    -- Exception path
    when (isSealedCapMem newCap .&&. inv (isSentryCapMem newCap)) do
      trap s cheri_exc_sealViolation
  ...
  when ((s.opcode `is` [JAL, JALR]) .||. branch) do
    let newType = if s.opcode `is` [JALR] then -1 else getTypeCapMem newCap
    s.pccNewCapMem <== setTypeCapMem newCap newType {- Unseal -}
      ...
      when (s.opcode `is` [LOAD, STORE, AMO]) do
        if inv memReqs.canPut
          then s.retry
            ...
            let (memCapTag :: Bit 1, memCap) = split cB
            -- Possible exceptions
            let exceptionTable =
                  [ inv (isValidCapMem cA)
...
executeBoundsUnit boundsUnit s = do
  when (s.opcode `is` [CSetBounds, CSetBoundsExact, CRRL, CRAM,
                         CGetPerm, CGetType, CGetBase, CGetLen,
                           CGetTag, CGetSealed, CGetFlags, CGetAddr]) do
